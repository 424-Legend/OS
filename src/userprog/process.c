...
{
  char *fn_copy;  // copy of FILE_NAME
  char *thread_name;  
  tid_t tid;
  ...
  thread_name = strtok_r (thread_name," ",&next_split_ptr);  // 新线程的名字
  /* Create a new thread to execute FILE_NAME. */
  //printf("%d\n", current_thread->tid);
  tid = thread_create (thread_name, PRI_DEFAULT, start_process, fn_copy);
  free(thread_name);   // 释放thread_name
  if (tid == TID_ERROR)
...
/* A thread function that loads a user process and starts it
   running. */
static void
start_process (void *file_name_)    // file_name_ 为文件名加参数（未分割过）
{
  ...
  struct thread * current_thread = thread_current();
  current_thread->parent->child_process_load_successfully = success;
...
process_wait (tid_t child_tid UNUSED)
{
  ...
  struct list_elem *elem = find_child_proc(child_tid);
  struct child_process *child = list_entry (elem, struct child_process, child_elem);
...
process_exit (void)
{
  ...
  while(!list_empty(&thread_current()->list_of_children_processes))
  {
    elem = list_pop_front(&thread_current()->list_of_children_processes);
    struct process_file *f = list_entry (elem, struct child_process, child_elem);
    free(f);
  }
  lock_release(&filesys_lock);
  // intr_set_level(old_level);
...
load (const char *file_name, void (**eip) (void), void **esp)
{
  ...
  char *next_split_ptr;
  fn_cp = strtok_r(fn_cp, " ", &next_split_ptr);
